...
i=1
cat $1 | grep -v '#' | grep -i '[a-z]' | grep ';' | while IFS=';' read -r nom couleur_feuille couleur_transparent ligne1 ligne2 ligne3 ligne4 extra
do
  ...
  if (($i % 2 == 0)); then
	...
	echo "erreur dans $premiere_feuille $deuxieme_feuille ..."
     fi
   fi
...
pdftk $dossier_print_feuilles/*.pdf cat output "res/impression/feuilles.pdf"
pdftk $dossier_print_transparents/*.pdf cat output "res/impression/transparents.pdf"
